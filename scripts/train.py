...
def main(cfg: DictConfig):
    ...
    if action_transform is not None:
        if action_transform == "rate":
            ...
            transforms.append(transform)
        elif action_transform.startswith("multidiscrete"):
    ...
    if eval_type and eval_type not in ["ctbr", "log_actions", "log_observations", "log_all"]:
        raise ValueError("eval_type must be either 'ctbr', 'log_actions', 'log_observations' or 'log_all'")
    ...
    if only_eval_one_traj and use_debug_policy==False and cfg.get("policy_checkpoint_path") is None:
        raise ValueError("policy_checkpoint_path is required for only_eval_one_traj without use_debug_policy")
    ...
    info.update(evaluate(policy=policy, only_eval_one_traj=only_eval_one_traj, eval_type=eval_type))
