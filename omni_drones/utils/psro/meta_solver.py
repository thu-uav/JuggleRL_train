# https://github.com/marlbenchmark/onpolicy/blob/magent/onpolicy/utils/meta_solver.py
...
class FSPSolver(MetaSolver):
    ...
    def solve(self, payoffs):
        ...
        for payoff in payoffs:
            oppo_population_size = payoff.shape[1]
            meta_strategy_list.append(np.ones(oppo_population_size) / oppo_population_size)
        return meta_strategy_list
...
class NashSolver(MetaSolver):
    ...
    def solve(self, payoffs):
        assert len(payoffs) == 2, f"Currently Nash solver only supports 2-player zero-sum game, but got {payoffs.shape[0]} players."
